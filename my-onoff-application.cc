...
// ggarciabas modification, allowing change stoptime, and use it!!!
...
void MyOnOffApplication::ScheduleNextTx ()
{
  ...
  // validate if the stop time has archieved!
  if (Simulator::Now().GetSeconds () >= m_stopTime.GetSeconds()) {
      NS_LOG_DEBUG ("-- Stop application OnOff!");
      StopApplication();
  }
