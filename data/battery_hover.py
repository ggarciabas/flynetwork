...
def battery(main_path, teste, uav_bat):
    ...
    for name_file in glob.glob(main_path+'uav_battery_hover_*.txt'):
        ...
        uav_id = "UAV "+os.path.splitext(base)[0].split("uav_battery_hover_")[-1]
        if teste:
    ...
    plt.savefig(main_path+'battery_hover.svg')
    plt.savefig(main_path+'battery_hover.eps')
    plt.savefig(main_path+'battery_hover.png')
