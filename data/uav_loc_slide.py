...
def slide (time, main_path, teste, title, folder):
    ...
    if len(glob.glob(main_path+time+'/f_mij_.eps')) == 0:
        return
    ...
    mij_file = os.path.basename(glob.glob(main_path+time+'/f_mij_.eps')[-1])
    ...
    copyfile(main_path+time+'/'+mij_file, main_path+'../../slide/'+folder+'/'+str(time)+'_'+mij_file)
    copyfile(main_path+time+'/'+bij_file, main_path+'../../slide/'+folder+'/'+str(time)+'_'+bij_file)
    copyfile(main_path+time+'/'+uav_loc_file, main_path+'../../slide/'+folder+'/'+str(time)+'_'+uav_loc_file)
