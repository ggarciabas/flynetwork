...
for custo in custos:
    ...
    list_folder.sort()
    data = {"Proposto": [], "Exaustivo":[], "Sequencial":[], "Aleatório":[]}
    for time in list_folder:
        ...
        data["Proposto"].append(c_prop.sum())
        data["Sequencial"].append(c_seq.sum())
        data["Aleatório"].append(c_ale.sum())

        if custo == "custo_1":
            data_c["Custo 1"].append(c_prop.sum())
        elif custo == "custo_2":
            data_c["Custo 2"].append(c_prop.sum())
        elif custo == "custo_3":
            data_c["Custo 3"].append(c_prop.sum())
