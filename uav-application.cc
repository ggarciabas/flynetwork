...
void UavApplication::Start(double stoptime) {
  NS_LOG_FUNCTION(this << Simulator::Now().GetSeconds()  << stoptime);
...
{
  NS_LOG_FUNCTION(this << Simulator::Now().GetSeconds()  << mob);
...
{
  NS_LOG_FUNCTION(this << Simulator::Now().GetSeconds()  << oldV << newV);
