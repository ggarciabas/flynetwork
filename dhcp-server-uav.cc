...
NS_LOG_COMPONENT_DEFINE ("DhcpServerUav");
NS_OBJECT_ENSURE_REGISTERED (DhcpServerUav);

TypeId
DhcpServerUav::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::DhcpServerUav")
    .SetParent<Application> ()
    .AddConstructor<DhcpServerUav> ()
    .SetGroupName ("Internet-Apps")
    .AddAttribute ("LeaseTime",
                   "Lease for which address will be leased.",
                   TimeValue (Seconds (30)),
                   MakeTimeAccessor (&DhcpServerUav::m_lease),
                   MakeTimeChecker ())
    .AddAttribute ("RenewTime",
                   "Time after which client should renew.",
                   TimeValue (Seconds (15)),
                   MakeTimeAccessor (&DhcpServerUav::m_renew),
                   MakeTimeChecker ())
    .AddAttribute ("RebindTime",
                   "Time after which client should rebind.",
                   TimeValue (Seconds (25)),
                   MakeTimeAccessor (&DhcpServerUav::m_rebind),
                   MakeTimeChecker ())
    .AddAttribute ("PoolAddresses",
                   "Pool of addresses to provide on request.",
                   Ipv4AddressValue (),
                   MakeIpv4AddressAccessor (&DhcpServerUav::m_poolAddress),
                   MakeIpv4AddressChecker ())
    .AddAttribute ("FirstAddress",
                   "The First valid address that can be given.",
                   Ipv4AddressValue (),
                   MakeIpv4AddressAccessor (&DhcpServerUav::m_minAddress),
                   MakeIpv4AddressChecker ())
    .AddAttribute ("LastAddress",
                   "The Last valid address that can be given.",
                   Ipv4AddressValue (),
                   MakeIpv4AddressAccessor (&DhcpServerUav::m_maxAddress),
                   MakeIpv4AddressChecker ())
    .AddAttribute ("PoolMask",
                   "Mask of the pool of addresses.",
                   Ipv4MaskValue (),
                   MakeIpv4MaskAccessor (&DhcpServerUav::m_poolMask),
                   MakeIpv4MaskChecker ())
...
void
DhcpServerUav::DoDispose (void)
{
  NS_LOG_FUNCTION (this);
  Application::DoDispose ();
}

void DhcpServerUav::StartApplication (void)
{
  NS_LOG_FUNCTION (this);
  ...
  m_socket->SetRecvCallback (MakeCallback (&DhcpServerUav::NetHandler, this));
  m_expiredEvent = Simulator::Schedule (Seconds (1), &DhcpServerUav::TimerHandler, this);
}

void DhcpServerUav::StopApplication ()
{
  NS_LOG_FUNCTION (this);
...
void DhcpServerUav::TimerHandler ()
{
  NS_LOG_FUNCTION (this);
...
void DhcpServerUav::NetHandler (Ptr<Socket> socket)
{
...
void DhcpServerUav::SendOffer (Ptr<NetDevice> iDev, DhcpHeader header, InetSocketAddress from)
{
...
void DhcpServerUav::SendAck (Ptr<NetDevice> iDev, DhcpHeader header, InetSocketAddress from)
{
  NS_LOG_FUNCTION (this << iDev << header << from);
...
void DhcpServerUav::AddStaticDhcpEntry (Address chaddr, Ipv4Address addr)
{
